(()=>{
'use strict';
/* DAILY SHOP v3 — integrated into the existing tools/settings menu */
...
#md-shop-btn{border:0;border-radius:14px;padding:10px 14px;background:linear-gradient(145deg,#f5b642,#e67e22);color:#3b2008;font-weight:900;font-size:14px;cursor:pointer;box-shadow:0 3px 0 #a85c18;margin:4px;width:100%;font-family:inherit}
#md-shop-btn:active{transform:translateY(2px);box-shadow:0 1px 0 #a85c18}
...
function findMenu(){
 const all=[...document.querySelectorAll('button,[role="button"],a,.menu-item,.settings-item,.tool-item')];
 const keys=['حالت شب','شب','تمام صفحه','پروفایل','آمار','تنظیمات','dark','fullscreen','profile','stats'];
 const hits=all.filter(el=>{const t=(el.innerText||el.textContent||'').trim().toLowerCase();return keys.some(k=>t.includes(k.toLowerCase()))});
 if(!hits.length)return null;
 const parents=hits.map(x=>x.parentElement).filter(Boolean);
 const score=p=>{const t=(p.innerText||'').toLowerCase();return keys.reduce((n,k)=>n+(t.includes(k.toLowerCase())?1:0),0)};
 return parents.sort((a,b)=>score(b)-score(a))[0]||hits[hits.length-1].parentElement;
}
function placeButton(){const b=document.getElementById('md-shop-btn');if(!b)return;const menu=findMenu();if(menu){if(b.parentElement!==menu)menu.appendChild(b);b.style.display='block';b.dataset.placed='1';return true}b.style.display='none';return false}
function build(){if(document.getElementById('md-shop')){placeButton();return}css();const b=document.createElement('button');b.id='md-shop-btn';b.textContent='🪙 فروشگاه و جایزه';b.onclick=openShop;b.style.display='none';document.body.appendChild(b);const o=document.createElement('div');o.id='md-shop';o.innerHTML=`<div class="box"><h2>🪙 فروشگاه مرغ‌دونی</h2><div class="wallet">موجودی: <span id="md-coins">0</span> 🪙</div><div class="daily"><strong>🎁 جایزه روزانه</strong><div id="md-daily-text">هر روز سکه رایگان بگیر!</div><button id="md-claim">دریافت ۱۰۰ سکه</button></div><div class="msg" id="md-msg"></div><div class="items" id="md-items"></div><button class="md-close" id="md-close">بستن</button></div>`;document.body.appendChild(o);o.querySelector('#md-close').onclick=()=>o.classList.remove('on');o.addEventListener('click',e=>{if(e.target===o)o.classList.remove('on')});o.querySelector('#md-claim').onclick=claim;renderItems([]);placeButton()}
function msg(t){const e=document.getElementById('md-msg');if(e)e.textContent=t||''}
function render(p){p=p||{};const coins=document.getElementById('md-coins');if(coins)coins.textContent=Number(p.coins||0).toLocaleString('fa-IR');const inv=p.inventory||[];const claim=document.getElementById('md-claim');if(claim){const today=p.today||new Date().toISOString().slice(0,10);claim.disabled=p.lastDaily===today;claim.textContent=p.lastDaily===today?'✅ جایزه امروز دریافت شد':'🎁 دریافت ۱۰۰ سکه'}const daily=document.getElementById('md-daily-text');if(daily)daily.textContent=`روزهای پیاپی: ${Number(p.streak||0).toLocaleString('fa-IR')} 🔥`;renderItems(inv)}
...
function start(){build();bind();placeButton();setInterval(()=>{bind();placeButton()},500)}
