...
function install(s){if(!s||socket===s)return;socket=s;
 ...
 s.on('gameRequestError',d=>{console.warn(d);render()});
 s.on('gameError',d=>console.warn(d));
...
function render(tab='players'){
 ...
 if(tab==='players'){
  ...
  req.forEach((r,i)=>{const row=document.createElement('div');row.className='mlg-request';row.innerHTML=`<div class="mlg-info"><div class="mlg-avatar">${esc(r.avatar||'🐔')}</div><div><div class="mlg-name">${esc(r.fromName||'بازیکن')}</div><div class="mlg-meta">نوع بازی: ${esc(r.gameType||'معمولی')}</div></div></div><div class="mlg-actions"><button class="mlg-btn mlg-accept">✅ قبول</button><button class="mlg-btn mlg-reject">❌ رد کردن</button></div>`;row.querySelector('.mlg-accept').onclick=()=>{socket?.emit('acceptGame',{fromId:r.fromId});incoming.splice(i,1);render('requests')};row.querySelector('.mlg-reject').onclick=()=>{socket?.emit('rejectGame',{fromId:r.fromId});incoming.splice(i,1);render('requests')};grid.appendChild(row)});
 }else{
  const rooms={};online.filter(p=>p?.status==='room'||p?.status==='playing').forEach(p=>{const key=p.roomId||p.gameId||'بازی‌های فعال';(rooms[key]??=[]).push(p)});
  body.innerHTML=`<section class="mlg-section"><h2>🏠 اتاق‌های فعال</h2><div class="mlg-grid" id="mrgRooms"></div></section>`;const grid=document.getElementById('mrgRooms');const keys=Object.keys(rooms);if(!keys.length)grid.innerHTML='<div class="mlg-empty">در حال حاضر اتاق فعالی در لیست عمومی پیدا نشد.</div>';keys.forEach(k=>{const ps=rooms[k];const row=document.createElement('div');row.className='mlg-room';row.innerHTML=`<div><div class="mlg-name">🏠 ${esc(k)}</div><div class="mlg-meta">${ps.length} بازیکن در حال بازی/انتظار</div></div><button class="mlg-btn mlg-watch">👀 مشاهده</button>`;row.querySelector('.mlg-watch').onclick=()=>{if(k&&k!=='بازی‌های فعال')socket?.emit('joinRoom',{roomId:k})};grid.appendChild(row)})
 }
 setLobbyBadge();
}
