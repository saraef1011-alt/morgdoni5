...
export class GameRoom extends BaseGameRoom {
  async message(id, raw) {
    ...
    if (t === 'joinRoom') {
      ...
      if (existing) {
        if (existing.roomId === rid) {
          return this.send(id, 'joinExistingGame', {
            roomId: rid,
            room: r,
            mode: existing.role === 'watcher' ? 'watcher' : 'player'
          });
        }
        return this.send(id, 'roomError', 'ابتدا از اتاق فعلی خارج شوید');
      }

      if ((r.players?.length || 0) >= MAX_PLAYERS) {
        return this.send(id, 'roomError', `ظرفیت اتاق پر است (حداکثر ${MAX_PLAYERS} بازیکن)`);
      }

      const p = online[id] || {
        id,
        name: String(d.playerName || 'بازیکن').slice(0, 20) || 'بازیکن',
        accountId: id,
        avatar: '🐔',
        status: 'ready'
      };
      ...
      if (r.gameStarted) {
        // ورود بازیکن جدید به بازی در حال اجرا: چهار کارت شروع می‌گیرد.
    ...
    // درخواست بازی برای بازیکنانی که از قبل داخل بازی هستند توسط BaseGameRoom
    // به busyGameChoice منتقل می‌شود؛ chooseGameOption نیز بدون محدودیت ۲ نفر
    // بازیکن را به همان اتاق اضافه می‌کند.
...
export default {
  async fetch(request, env) {
    ...
    if (u.pathname === '/' || u.pathname === '/index.html' || type.includes('text/html')) {
      ...
      if (!/morgdoni-card-fix\.js/i.test(html)) html = html.replace(/<\/body>/i, '<script src="/morgdoni-card-fix.js?v=multi-final"></script></body>');
      if (!/socket\.io\/socket\.io\.js/i.test(html)) html = html.replace(/<\/head>/i, '<script src="/socket.io/socket.io.js"></script></head>');
      if (!/request-ui\.js/i.test(html)) html = html.replace(/<\/body>/i, '<script src="/request-ui.js?v=multi-final"></script></body>');
