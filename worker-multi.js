...
// همان GameRoom قبلی؛ فقط ورود مستقیم به اتاق دیگر محدود به ۲ نفر نیست.
export class GameRoom extends BaseGameRoom {
  async message(id, raw) {
    ...
    if (t === 'joinRoom') {
      ...
      const p = online[id] || {
        id,
        name: String(d.playerName || 'بازیکن').slice(0, 20),
        accountId: id,
        avatar: '🐔'
      };
      online[id] ??= p;
      ...
      // حتی اگر بازی شروع شده باشد، بازیکن جدید با حداکثر ۴ کارت وارد می‌شود.
      if (r.gameStarted) {
    ...
    // تمام رویدادهای قبلی بدون تغییر اجرا می‌شوند؛ از جمله درخواست بازی،
    // جوین/تماشا، کارت‌ها، AI، چت، پروفایل و WebRTC.
...
export default {
  async fetch(request, env) {
    ...
    if (u.pathname === '/socket.io/socket.io.js') {
      return new Response(SOCKET_SHIM, {
        headers: {
    ...
    if (u.pathname === '/ws') {
      return env.GAME_ROOM.get(env.GAME_ROOM.idFromName('morgdoni-lobby')).fetch(request);
    }
