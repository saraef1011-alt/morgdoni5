...
export class GameRoom extends BaseGameRoom {
  async message(id, raw) {
    ...
    // Multiplayer room join: allow players 2..50, both before and after start.
    if (t === 'joinRoom') {
      ...
      if (existing) {
        if (existing.roomId === rid) {
          return this.send(id, 'joinExistingGame', { roomId: rid, room: r, mode: existing.role === 'watcher' ? 'watcher' : 'player' });
        }
    ...
    // Create a room without a 2-player cap. The room can later grow to 50 players.
    if (t === 'createRoom') {
      ...
      online[id].status='room';
      this.send(id,'roomCreated',{roomId:rid});
      this.roomBroadcast(this.data.rooms[rid],'roomUpdate',this.data.rooms[rid]);
      this.updateList();
      await this.save();
      return;
    }

    // Explicitly handle requests here so a player can request ANY other online player,
    // including players already in a 3rd/4th/5th/... player game.
    if (t === 'requestGame') {
      ...
      this.send(id,'gameRequestSent',{targetId:target.id,targetName:target.name});
      this.updateList();
      await this.save();
      return;
    }

    // Everything else (accept/reject/choose/join-existing/game actions/profile/etc.)
    // uses the already multiplayer-capable base implementation.
...
export default {
  async fetch(request, env) {
    ...
    if(u.pathname==='/'||u.pathname==='/index.html'||type.includes('text/html')){
      ...
      if(!/morgdoni-card-fix\.js/i.test(html))html=html.replace(/<\/body>/i,'<script src="/morgdoni-card-fix.js?v=multi-final2"></script></body>');
      if(!/socket\.io\/socket\.io\.js/i.test(html))html=html.replace(/<\/head>/i,'<script src="/socket.io/socket.io.js"></script></head>');
      if(!/request-ui\.js/i.test(html))html=html.replace(/<\/body>/i,'<script src="/request-ui.js?v=multi-final2"></script></body>');
