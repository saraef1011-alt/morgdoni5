...
.morgdoni-request-btn{border:0;border-radius:22px;padding:12px 18px;margin:5px;font-weight:bold;font-size:1rem;cursor:pointer;min-width:105px}.mrg-green{background:#27ae60;color:white}.mrg-blue{background:#3498db;color:white}.mrg-red{background:#c0392b;color:white}.mrg-gray{background:#7f8c8d;color:white}.morg-online-list{display:grid;gap:7px;text-align:right}.morg-online-row{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:8px 10px;border:1px solid #ead5b0;border-radius:15px;background:#fffaf0}.morg-online-row button{border:0;border-radius:14px;padding:7px 10px;background:#27ae60;color:#fff;font-weight:bold;cursor:pointer}.morg-status{font-size:.75rem;color:#777}
...
function renderOnline(list){
 ...
 if(!panel){
  panel=document.createElement('div');panel.id='morgOnlinePanel';panel.className='morgdoni-request-box';panel.style.cssText='position:fixed;right:14px;bottom:14px;width:min(430px,94vw);z-index:99990;padding:16px;';
 ...
 rows.forEach(x=>{
  ...
  const info=document.createElement('span');info.innerHTML=`${esc(x.avatar||'🐔')} ${esc(x.name||'بازیکن')} <span class="morg-status">(${esc(x.status||'ready')})</span>`;
  const b=document.createElement('button');b.textContent='📨 درخواست بازی';b.onclick=()=>{if(socket)socket.emit('requestGame',{targetId:x.id});};
