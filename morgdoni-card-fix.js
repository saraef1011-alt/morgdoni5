...
css.textContent = `
/* FIX: کارت‌ها کامل و بدون بریده‌شدن نمایش داده شوند */
...
function installSocket(s){
 ...
 s.on('gameRequest',d=>{currentRequest=d||{};modal('📨 درخواست بازی',`${esc(currentRequest.fromName||'یک بازیکن')} می‌خواهد با شما بازی کند.`,[{text:'🎮 قبول',cls:'mrg-green',click:()=>s.emit('acceptGame',{fromId:currentRequest.fromId})},{text:'❌ رد کردن',cls:'mrg-red',click:()=>s.emit('rejectGame',{fromId:currentRequest.fromId})}])});
 s.on('busyGameChoice',d=>{currentBusy=d||{};modal('👥 ورود به بازی',`${esc(currentBusy.fromName||'یک بازیکن')} می‌خواهد وارد بازی فعلی شما شود.`,[{text:'🎮 جوین',cls:'mrg-green',click:()=>s.emit('chooseGameOption',{fromId:currentBusy.fromId,option:'join'})},{text:'👀 تماشا',cls:'mrg-blue',click:()=>s.emit('chooseGameOption',{fromId:currentBusy.fromId,option:'watch'})},{text:'❌ رد کردن',cls:'mrg-red',click:()=>s.emit('rejectGame',{fromId:currentBusy.fromId})}])});
 ...
 s.on('playerListUpdate',list=>renderOnline(list||[])); s.on('registrationSuccess',()=>setTimeout(()=>s.emit('getPlayerList'),100)); s.emit('getPlayerList');
}
function renderOnline(list){let panel=document.getElementById('morgOnlinePanel');if(!panel){panel=document.createElement('div');panel.id='morgOnlinePanel';panel.className='morgdoni-request-box';panel.style.cssText='position:fixed;right:14px;bottom:14px;width:min(430px,94vw);z-index:99990;padding:16px;';document.body.appendChild(panel)}const me=socket?.id;const rows=(Array.isArray(list)?list:[]).filter(x=>x.id&&x.id!==me);panel.innerHTML='<h2>👥 بازیکنان آنلاین</h2><div class="morg-online-list"></div>';const wrap=panel.querySelector('.morg-online-list');if(!rows.length){wrap.innerHTML='<div style="padding:10px;text-align:center">بازیکن دیگری آنلاین نیست</div>';return}rows.forEach(x=>{const row=document.createElement('div');row.className='morg-online-row';const info=document.createElement('span');info.innerHTML=`${esc(x.avatar||'🐔')} ${esc(x.name||'بازیکن')} <span class="morg-status">(${esc(x.status||'ready')})</span>`;const b=document.createElement('button');b.textContent='📨 درخواست بازی';b.onclick=()=>{if(socket)socket.emit('requestGame',{targetId:x.id})};row.append(info,b);wrap.appendChild(row)})}
...
watchSocket();setInterval(watchSocket,300);if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>paintCards());else paintCards();
